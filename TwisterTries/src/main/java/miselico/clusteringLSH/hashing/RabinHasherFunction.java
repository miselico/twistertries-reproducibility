 ...
 * 
 */
public class RabinHasherFunction extends AbstractStreamingHashFunctionMadePublic {

	private final long[] pushTable;
	private final int degree;
	private final int shift;

	public RabinHasherFunction(long polynomial) {
		Polynomial poly = Polynomial.createFromLong(polynomial);
		this.pushTable = RabinHasherFunction.precomputePushTable(poly);
	...
	//		return super.newHasher(expectedInputSize);
	//	}

	@Override
	public Hasher newHasher() {

		return new AbstractStreamingHasherMadePublic(RabinHasherFunction.hashBufferSize) {
			...
			protected void process(ByteBuffer bb) {
				...
				for (int i = 0; i < RabinHasherFunction.hashBufferSize; i++) {
			...
			public HashCode makeHash() {
				//FIXME This could be tried to optimize something. Some space should be saved and comparison of hashes is faster
				//if (RabinHasherFunction.this.bits() < 32) {
				//	return HashCode.fromInt((int) this.fingerprint);
				//} else {
			...
			private void pushByte(byte b) {
				int j = (int) ((this.fingerprint >> RabinHasherFunction.this.shift) & 0x1FF);
				this.fingerprint = ((this.fingerprint << 8) | (b & 0xFF)) ^ RabinHasherFunction.this.pushTable[j];
			}
		};
	}
