 ...
 * 
 * <p>
 * Conceals use of <code>System.currentTimeMillis()</code>, improving the readability of application code and reducing the likelihood of calculation errors.
 * 
 * <p>
...
public class StopWatch {
	...
	/**
	 * Determine whether the TaskInfo array is built over time. Set this to "false" when using a StopWatch for millions of intervals, or the task info structure will consume excessive memory. Default
	 * is "true".
	 */
	...
	/**
	 * Start an unnamed task. The results are undefined if {@link #stop()} or timing methods are called without invoking this method.
	 * 
	 * @see #stop()
	 */
	...
	/**
	 * Start a named task. The results are undefined if {@link #stop()} or timing methods are called without invoking this method.
	 * 
	 * @param taskName
	...
	public void start(String taskName) throws IllegalStateException {
		if (this.running) {
			...
			try {
				new Error("Carefull, timer was still running");
			} catch (Error a) {
				a.printStackTrace();
			}
			this.stop();
	...
	public void stop() throws IllegalStateException {
		if (!this.running) {
			...
			try {
				new Error("Carefull, timer was not running");
			} catch (Error a) {
				a.printStackTrace();
			}
			this.stop();
	...
	public void stopStart(String taskName) {
		this.stop();
		this.start(taskName);
	}
	...
	public String prettyPrint() {
		...
		if (!this.keepTaskList) {
			...
			for (int i = 0; i < tasks.length; i++) {
				...
				sb.append(pf.format(tasks[i].getTimeSeconds() / this.getTotalTimeSeconds()) + "  ");
	...
	/**
	 * Return an informative string describing all tasks performed For custom reporting, call <code>getTaskInfo()</code> and use the task info directly.
	 */
	@Override
